...
This is the mandatory execution protocol for EVERY weDO milestone from M3
onward.

Both developers and all AI coding agents MUST follow the same workflow.

A milestone may have a different owner, but it must NOT have a different
engineering process.

This document supplements:

docs/AI_AGENT_RULES.md
latest AI_AGENT_HANDOFF_*.md
docs/DEVELOPMENT_PLAN_MILESTONES_v1.0_FULL.md

If rules conflict:

AI_AGENT_RULES.md remains authoritative for permanent operational/security
rules.
...
MILESTONE OWNER
→ primarily responsible for analysis, implementation, tests, documentation,
  integration evidence, and final report.
...
→ required at architecture/security/database boundaries as defined by
  AI_AGENT_RULES.md.
...
Both teammates should understand the important API, schema, business rules and
architecture before the milestone is merged.
...
DEPENDENCIES:
which previous milestone contracts are required
...
A milestone does NOT automatically have to wait until every previous numbered
...
→ required schema, API, service contracts and business rules from dependencies
  are stable enough to build against.

INTEGRATION READY
→ required dependencies are merged and runnable, so full E2E/integration can
  be proven.
...
M6 production implementation = NOT ALLOWED until required M5 contracts are
stable.
...
the Flutter implementation and dependent milestones may now safely build
...
if only backend unit tests were executed.

Do not claim:

physical-device tested

unless a device was actually used.
...
If the team has assigned an Integration Owner, that person should perform or
approve shared wiring.
...
Do not commit directly to main unless the team explicitly changes its Git
strategy.
...
Why do those constraints exist?

What is the HTTP flow?

Why is business logic in the Service?

Where is the transaction boundary?

What can race?

What security check exists?

How does Flutter call the backend?
...
How would you rebuild the same pattern in another project?

How would you explain this milestone in a Backend Intern interview?

If the owner cannot explain these questions, the learning goal of the milestone
is not complete even if the code runs.
